...
static irqreturn_t bflb_spi_isr(int irq, void *dev_id)
{
	...
	if (pending & SPI_INT_STS_RXF_RDY) {
		 ...
		 */
		u32 consumed = bflb_spi_rxfifo_drain(spi);
		if (!spi->rx_len) {
			...
			/*
			 * check if there is some tx work to do. Although the driver
			 * has tried to drain the rx fifo, the spi controller is still
			 * working concurrently, which means there could be new data in
			 * the rx fifo and tx fifo has more room for transfer right now.
			 * Rx fifo overflow could occur if the driver tries to push as
			 * much data as possible, and that's why the variable consumed
			 * is the clamp.
			 */
								...
								struct spi_transfer *t)
{
	...
	dev_dbg(spi->dev, "xfer len %u, stride %u\n", spi->tx_len, spi->xfer_stride);
    if (readl(spi->regs + SPI_BUS_BUSY_REG_OFFSET))
        dev_warn(spi->dev, "note: spi bus is busy?!\n");
	return 0;
}

static int bflb_spi_transfer_one_poll(struct spi_controller *ctlr,
...
static size_t bflb_spi_get_max_xfer_size(struct spi_device *spi_dev)
{
...
static int bflb_spi_debugfs_deinit(struct bflb_spi *spi)
{
...
static struct clk_ops bflb_spi_clk_ops = {
...
static int bflb_spi_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(spi->pclk)) {
        ...
        return dev_err_probe(spi->dev, ret, "unable to get enable clock\n");
	}
	...
	of_property_read_u32(np, "bflb,max-bits-per-word", &spi->max_bits_per_word);
	...
	//ctlr->transfer_one = bflb_spi_transfer_one_poll;
	ctlr->transfer_one = bflb_spi_transfer_one_irq;
	//ctlr->max_transfer_size = bflb_spi_get_max_xfer_size;
	...
	dev_info(spi->dev, "mapped; irq=%d, cs=%d\n",
             irq, ctlr->num_chipselect);
...
static void bflb_spi_remove(struct platform_device *pdev)
{
    ...
    u32 tmp = SPI_INT_CLR_ALL | SPI_INT_MASK_ALL;
    writel(tmp, spi->regs + SPI_INT_REG_OFFSET);
	clk_disable(spi->pclk);
	bflb_spi_debugfs_deinit(spi);
}
